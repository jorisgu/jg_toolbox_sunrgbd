...
% for ii = 10000:10010
    ...
    %     a_EGH(:,:,2)=G;
    %     a_EGH(:,:,3)=H;
    %     saveIt( a_EGH, data_path, 'egh_8bits', a_name, a_extension);
    %     disp('rgbd_egh_8bits')
    
    %% rgbd tiff
    %     a_RGBD=zeros(size(a_d_raw,1),size(a_d_raw,2),4,'double');
    %     a_RGBD(:,:,1:3)=a_rgb/255.0;
    %     a_RGBD(:,:,4)=a_d_raw/65535.0;
    %     saveIt( a_EGH, data_path, 'rgbd_range01', a_name, 'tiff');
